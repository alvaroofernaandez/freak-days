...
import { Check, Copy, Crown, Eye, LogOut, Users } from 'lucide-vue-next'
...
<template>
  <Card
    class="group hover:border-primary/40 transition-all duration-300 hover:shadow-xl hover:shadow-primary/10 hover:-translate-y-1 relative overflow-hidden"
  >
    <div
    ...
    <CardHeader class="relative space-y-3 sm:space-y-4 p-4 sm:p-6">
      <div class="flex items-start gap-2 sm:gap-3">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-1.5 sm:gap-2 mb-1">
            ...
            <Crown
              v-if="isOwner"
              class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-exp-medium shrink-0 animate-pulse"
              aria-label="Eres el dueño de esta party"
            />
          </div>
          <CardDescription class="text-xs flex items-center gap-1 sm:gap-1.5 flex-wrap">
            <Users class="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary/70 shrink-0" />
      ...
      >
        <code
          class="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 bg-background/80 rounded-md text-xs font-mono font-bold text-center tracking-wider sm:tracking-widest text-primary group-hover:bg-background transition-colors"
        ...
        <Button
          variant="ghost"
          size="icon"
          class="h-9 w-9 sm:h-8 sm:w-8 shrink-0 hover:bg-primary/10 hover:text-primary transition-colors touch-manipulation"
          @click="emit('copyCode', party.inviteCode!)"
          :aria-label="copiedCode === party.inviteCode ? 'Código copiado' : 'Copiar código de invitación'"
        >
          <Check
      ...
      <div class="flex items-center justify-between gap-2 pt-2 sm:pt-3 border-t border-border/50">
        ...
        <div class="flex gap-1">
          <Button
            variant="ghost"
            size="icon"
            class="h-9 w-9 sm:h-8 sm:w-8 hover:bg-primary/10 hover:text-primary transition-colors touch-manipulation"
            @click="emit('viewDetails', party)"
            aria-label="Ver detalles de la party"
          >
            <Eye class="h-4 w-4" />
          </Button>
          <Button
            ...
            variant="ghost"
            size="icon"
            class="h-9 w-9 sm:h-8 sm:w-8 text-destructive hover:text-destructive hover:bg-destructive/10 transition-colors touch-manipulation"
            @click="emit('leave', party.id)"
