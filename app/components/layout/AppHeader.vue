...
<template>
  <header
    class="sticky top-0 z-50 w-full border-b border-border/50 bg-background/95 backdrop-blur-xl lg:block hidden shadow-sm"
    style="position: sticky; z-index: 9999; pointer-events: auto;">
    <div class="container mx-auto flex h-16 items-center px-4">
      <NuxtLink to="/" class="flex items-center gap-3 font-bold group">
        <div class="relative flex items-center justify-center">
          ...
          <div
            class="relative p-2 rounded-xl bg-linear-to-br from-primary/10 via-accent/5 to-primary/10 border border-primary/20 group-hover:border-primary/40 group-hover:bg-primary/15 transition-all duration-300 shadow-sm">
            <img src="/logo.png" alt="FreakDays"
              class="h-8 w-8 rounded-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3"
      ...
      <div class="ml-auto flex items-center gap-3">
        ...
        <NuxtLink v-if="profile" to="/profile"
          class="flex items-center gap-3 pl-4 border-l border-border/50 group/profile">
          ...
          <div class="relative">
            ...
            <Avatar
              class="relative h-10 w-10 ring-2 ring-transparent group-hover/profile:ring-primary/50 transition-all shadow-md">
              <AvatarImage v-if="profile?.avatarUrl" :src="profile.avatarUrl"
                :alt="profile?.displayName || profile?.username" class="object-cover" />
              <AvatarFallback class="bg-linear-to-br from-primary to-accent text-sm text-white font-bold">
