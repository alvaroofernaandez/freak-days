...
import { AlertTriangle, Home, RefreshCw, ArrowLeft, Search, Settings, Lock, User, Clock, Wrench, X } from 'lucide-vue-next'
    ...
    <Card class="w-full max-w-2xl border-destructive/20 shadow-2xl glass relative overflow-hidden">
      ...
      <CardHeader class="text-center space-y-4 relative z-10">
        <div class="relative mx-auto inline-flex items-center justify-center">
          <div class="absolute -inset-4 bg-destructive/20 blur-2xl rounded-full animate-pulse" style="animation-duration: 3s" />
          <div class="relative w-24 h-24 rounded-full bg-linear-to-br from-destructive/20 via-destructive/10 to-destructive/5 flex items-center justify-center border border-destructive/30 backdrop-blur-sm">
            <component :is="errorInfo.icon" class="h-12 w-12 text-destructive" />
          </div>
        </div>
        <div>
          <CardTitle class="text-4xl font-gamer mb-2 text-gradient">
            {{ error.value.statusCode || 'Error' }}
          </CardTitle>
      ...
      <CardContent class="space-y-6 relative z-10">
        ...
        <div v-if="error.message && error.statusCode !== 404" class="p-4 rounded-lg bg-muted/50 border border-destructive/20 backdrop-blur-sm relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Button @click="goHome" variant="outline" class="flex-1 sm:flex-none hover:bg-primary/10 hover:border-primary/30 hover:text-primary transition-all duration-300">
            <Home class="h-4 w-4 mr-2" />
            Inicio
          </Button>
          <Button @click="handleError" class="flex-1 sm:flex-none glow-primary hover:glow-primary hover:scale-105 transition-all duration-300">
